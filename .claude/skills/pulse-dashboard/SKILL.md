...
description: >
  ...
  Notification moved 2026-09-11: the "📍 Pulse updated" line posts ONLY to Pulse's own
  `#pulse` channel now — no longer DMed to Convo 1.
  ...
  and writes only the Drive snapshot — it sends the new link to `#pulse` only when this
  run actually changed something (see Notification below), never sets reactions, never
  edits the vault.
---
...
dashboard" section: the **Pulse Drive folder id** (create target) and the **`#pulse`
channel id** `C0C1A8PS3GS` (the ONLY notification target — Convo 1 no longer gets this
ping), plus the **self-DM id** `D0BBVV54L5R` (the quick-capture surface), the **workout
artifact URL** and the **Morning Brief Drive folder** (for the North Star link-out), the
Slack channel IDs, the reminder calendar ID, and the vault paths you already know from
the run.
...
## Notification — post to #pulse only when something changed
...
skip the render entirely — no Doc, no post (codified 2026-08-15; a folder of identical
snapshots is noise, not history) — and return `pulse — carried (quiet pass)` for the
digest. If YES to any, render, then send ONE line to **`#pulse`** (`C0C1A8PS3GS`, its
own dedicated channel — anchors, "Pulse dashboard" section). **Convo 1 no longer gets
this ping (moved 2026-09-11, per Lemar)** — the notification lives exclusively in
`#pulse` now:
...
Pulse Drive folder; send ONE line to `#pulse` (`C0C1A8PS3GS`), and only when something
changed this run.
You MUST NOT, ever: post to any Slack channel other than `#pulse` (Convo 1 included —
the DM ping was retired 2026-09-11); post on a quiet hour (nothing changed); send email
or any outreach; set or change Lemar's reactions; write, move, edit, or file any vault
note; touch the calendar; advise on, move, or commit money; run vault-keeper or
...
`pulse ✅ <Drive doc URL> · sections OK K/9 · notify sent/skipped (#pulse) · <list any errored sections>`
