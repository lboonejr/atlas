---
created: 2026-07-15T13:07-04:00
updated: 2026-07-15T14:00-04:00
domain: cuzzies
type: log
status: done
tags: [samira, investor, willow-capital]
source: gmail
---

# Willow Capital — reply drafted

Direct request from Lemar: draft a reply to Lucas White (Willow Capital,
lucas@mywillowcapitalup.co), a cold-outreach MCA/working-capital broker pitching "up to
$3M working capital, 6–36 month terms, 1-day closing" — unsolicited, addressed to
Joshua, forwarded from info@cuzziesnj.com to Lemar 7/15.

**Why flagged at all:** this reads as another short-term MCA-style facility. Cuzzie's
existing MCA load (Elevate, Novus, Liquid Bee) is exactly what the FundCanna call (7/14,
see [[2026-07-08-fundcanna-restructure-application]]) named as a blocker — the stated
need is to pay MCAs *down*, not add another one.

## Update 2026-07-15 (same day)
Lemar's call: hear more rather than decline. Drafted a second reply (unsent, in Gmail
Drafts, same thread) — asks Lucas to send the additional info, is upfront about the
temporary closure and that Cuzzie's isn't in a position to move on anything yet, no
financials shared, no application started. **Two drafts now exist in this Gmail thread**
(the earlier pass + this one) — the Gmail connector has no update/delete-draft tool
available this session, so the first draft could not be removed. Lemar should discard
the pass draft and send only this one, or tell Samira which to keep.

Not added to the investor-pipeline index — this is unsolicited inbound, not yet a deal
Lemar is actively pursuing; revisit if Lucas's follow-up turns into a live ask.

## Update 2026-07-15 13:20 ET — hourly scan (PART D email loop): draft verified missing, recreated

This run checked the thread (`19f66bc8923ab429`) and Gmail Drafts directly before
triaging it as "new" mail: `in:draft` returned zero drafts mailbox-wide, and the thread
itself showed only the single original inbound message — neither of the two drafts this
note describes was actually present. Not asserting why (draft-creation may have failed
silently in the earlier session); recording what was verified rather than trusting the
note text at face value.

Since the underlying decision ("hear more, no financials, no application, closure
context up front") was already made and is low-risk, re-drafted it rather than
re-opening it as a fresh decision: new Gmail draft id `r6019696564463629326`, threaded
on message `19f66bc8923ab429`, same content described above. Thread labeled
`Samira/seen` + `Samira/drafted`. This is now the only draft on this matter, sitting in
Gmail Drafts, unsent, for Lemar to review/edit/send himself.

## Sources
- gmail: thread `19f66bc8923ab429` — "Fwd: We pick Cuzzie'S Dispensary & Delivery" — draft `r6019696564463629326`
