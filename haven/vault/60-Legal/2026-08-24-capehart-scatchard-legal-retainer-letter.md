---
created: 2026-08-24T15:57:29Z
updated: 2026-08-25T12:06:41Z
domain: legal
type: task
status: active
tags: [capehart-scatchard, legal-retainer, litigation]
source: gmail
---

# Capehart Scatchard — legal retainer letter, $5,000, litigation already active

Laurel B. Peltzman (Shareholder, Labor and Employment Dept, Capehart & Scatchard, P.A.,
Mount Laurel NJ) emailed a retainer letter following a phone call with Lemar this
morning (2026-08-24, call not logged elsewhere in the vault — Lemar's own outreach,
outside Samira's visibility until this email).

**What's needed:** a signed copy of the attached retainer letter (`16534948.pdf`) and
payment of a **$5,000 retainer** — the letter states "this matter is already in
litigation." Bills are paid from the retainer monthly; unused funds returned; once
exhausted, billed monthly thereafter.

**Unknowns:** which matter this is (a Labor and Employment litigation — not yet
described anywhere else in the vault; distinct from the DeWalt collections matter,
which is being routed to Douglas Diaz at Archer & Greiner, and distinct from the
Camden County Bar referral to Salvatore Siciliano). Whether this is a Cuzzie's or
Station matter, and who the opposing party is, are both unknown — the phone call
that set this up isn't recorded anywhere Samira can see.

**Status:** Not signed, not paid, not authorized by Samira — signature and a $5,000
payment are both outward-facing and stay Lemar's call.

## Update — 2026-08-25T12:06:41Z
Lemar had already replied (outside a Samira draft — sent 2026-08-24 22:04 ET) flagging
that his business partner Josh Evans is also individually named as a defendant and would
need representation too. Laurel replied 2026-08-25 07:23am ET:
- Raises the retainer to **$10,000** (was $5,000) to cover both of them, same hourly rate.
- Needs confirmation there's **no conflict** between Lemar's and Josh's defenses/strategy
  — if their accounts of what happened could point at each other, she can't represent
  both.
- Needs **Josh's contact info** (address, email, phone) to send him his own retainer
  letter.
None of this is something Samira can answer (conflict-of-interest read, a partner's
contact info, a $10k payment) — raised as a #decisions card rather than drafted.
⚠️ counsel on thread.

## Sources
- gmail: thread `1a0347e284ea8f6f` ("Legal Retainer Letter", LPeltzman@capehart.com,
  cc CCaruso@capehart.com, 2026-08-24 15:57 UTC)
- gmail: same thread, message `1a038a9a6b444ff2` (Laurel Peltzman, 2026-08-25 11:23 UTC,
  $10,000 retainer + conflict check + Josh's info requested)
